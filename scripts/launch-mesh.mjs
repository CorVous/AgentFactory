 ...
 * Called on tail-toggle and tail-event to show the bus monitoring strip.
 * mesh-rail (in the focused peer's pi session) handles all peer-status display.
 */
...
//   1. Call focusController.setPeerState(peer, "crashed") to update state.
...
// unregisterPeer. mesh-rail (in the focused peer's pi session) renders the
// updated peer state via the mesh-rail-update broadcast.
...
launcherSock.on("envelope", (env) => {
  if (env.kind === "focus-request") {
    ...
    // Route through focusController so the mesh-rail broadcast carries the updated flag.
  ...
  pool.on("exit", (exitedName, code, signal) => {
    if (exitedName === name) {
      ...
      if (!isCrash) {
        ...
        // focusController.unregisterPeer triggers mesh-rail-update broadcast.
