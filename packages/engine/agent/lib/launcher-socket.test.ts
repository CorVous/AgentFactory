...
import type { LauncherEnvelope } from "./launcher-socket.mjs";
import {
...
describe("LauncherSocket — inbound envelope dispatch", () => {
  it("emits 'envelope' when a valid envelope arrives from a client", async () => {
    ...
    const received: LauncherEnvelope[] = [];
    server.on("envelope", (env: LauncherEnvelope) => received.push(env));
...
describe("LauncherSocket — broadcast", () => {
  it("broadcasts an envelope to all connected clients", async () => {
    ...
    const receivedA: LauncherEnvelope[] = [];
    const receivedB: LauncherEnvelope[] = [];
    clientA.on("envelope", (e: LauncherEnvelope) => receivedA.push(e));
    clientB.on("envelope", (e: LauncherEnvelope) => receivedB.push(e));
