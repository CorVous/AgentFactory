...
//
// Used by peer-bus.ts and launcher-bridge.ts to guard their session_start
 ...
 *   - messagesWith.length > 0
 *   - acceptsWorkFrom.length > 0
 *   - supervisor is a non-empty string
 *   - submitsWorkTo is a non-empty string
 *
...
export function habitatHasPeers(h: Habitat): boolean {
  if (Array.isArray(h.messagesWith) && h.messagesWith.length > 0) return true;
  if (Array.isArray(h.acceptsWorkFrom) && h.acceptsWorkFrom.length > 0) return true;
  if (h.supervisor && h.supervisor.trim().length > 0) return true;
  if (h.submitsWorkTo && h.submitsWorkTo.trim().length > 0) return true;
  return false;
}
