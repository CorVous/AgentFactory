...
interface PendingEntry {
  id: string;
  recipe: string;
  // Pre-generated by agent-spawn at delegate-time so the box title shows
  // the unique slug from the moment of dispatch, before the first status
  // envelope arrives. Optional because legacy callers may not set it.
  agentName?: string;
  startedAt: number;
...
function renderBox(theme: Theme, entry: PendingEntry, boxWidth: number): string[] {
  ...
  const rawName = entry.agentName ?? status?.agentName ?? entry.recipe;
  const name = prettify(rawName);
