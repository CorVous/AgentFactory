...
| 9 | **Sandbox root + verb selection.** `PI_SANDBOX_ROOT: sandboxRoot` AND `PI_SANDBOX_VERBS: <comma-sep subset>` in the child's `env` whenever `cwd-guard.ts` is loaded. The runner sets both automatically when you go through `makeCwdGuard({verbs: [...]})`. Compute `sandboxRoot` from the parent extension's own `import.meta.url`, not from `process.cwd()` or `$HOME`. | `defaults.md#for-every-writer-mutator` |
