...
| 8 | **Tool allowlist.** `tools` is the union of each user-listed component's `toolsContribution` plus any `sandbox_*` verbs the role needs. The runner activates `sandbox-fs` automatically when it sees a sandbox verb. Never include built-in `read`/`ls`/`grep`/`glob`/`write`/`edit` (forbidden across the project) or `bash` — those defeat every stub in the library. Also never list `cwd-guard` or `sandbox-fs` in `components:` (auto-injected). | `defaults.md#tool-allowlist-always-on` |
| 9 | **Sandbox root + verb selection.** `cwd-guard.ts` is auto-injected on EVERY sub-pi spawn (defense-in-depth) — sets `PI_SANDBOX_ROOT` and attaches a path-arg auditor. `sandbox-fs.ts` is auto-injected iff `tools` contains a `sandbox_*` verb; the runner derives the verb subset from `tools ∩ {sandbox_read,sandbox_ls,sandbox_grep,sandbox_glob,sandbox_write,sandbox_edit}` and pins `PI_SANDBOX_VERBS` automatically. For no-fs roles, simply omit sandbox verbs from `tools` and sandbox-fs doesn't load. Compute `sandboxRoot` from the parent extension's own `import.meta.url`, not from `process.cwd()` or `$HOME`. | `defaults.md#for-every-writer-mutator` |
