...
Closed via Ralph-Loop AFK trunk on `claude/issue-triage-automation-RCxDm`. Implementation merge: `feature/v1-ralph-loop-mesh-02-thin-orchestrator` → workflow branch (`--no-ff`). New recipe `pi-sandbox/agents/ralph/orchestrator-thin.yaml`, helper `_lib/issue-file.ts` (4 pure functions), 30 hermetic tests. `npm test` 242/242. `ralph` added to `AGENT_SUBDIRS`.
