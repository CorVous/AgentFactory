...
Thin Orchestrator recipe + helpers landed. `npm test` green at 243 tests (was 212).
Four pure helpers in `_lib/issue-file.ts`: `nextIssueNN` (NN from dir listing), `titleToSlug` (kebab slug), `buildIssueBody` (canonical issue markdown), `composeIssueFile` (composes all three into `{path, content}` ready for `deferred_write`).
