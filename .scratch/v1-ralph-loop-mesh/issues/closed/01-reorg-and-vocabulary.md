...
Closed via Ralph-Loop AFK trunk on `claude/issue-triage-automation-RCxDm`. Implementation merge: `feature/v1-ralph-loop-mesh-01-reorg-and-vocabulary` → workflow branch (`--no-ff`). Verification: `npm test` 212/212 pass; stale-reference grep clean; file layout matches AC.
