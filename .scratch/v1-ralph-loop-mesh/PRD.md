...
2. As a developer, I want the mesh launcher to fail loudly if the project has no `.scratch/<feature-slug>/issues/` directory on `feature/<feature-slug>`, so that I do not launch the Kanban against an unauthored feature (the orchestrator agent should run first to populate it — see stories 2d/2e).
2a. As a developer, I want the mesh launcher to create (or resume) `feature/<feature-slug>` off `main` and `git worktree add` it at `~/.mesh-scratch/<project>/<feature-slug>/kanban/`, so that the Kanban runs in a worktree fully isolated from my project-root checkout and from any colleague's parallel work on the same repo.
...
2d. As a developer, I want a new **orchestrator** pi recipe (`pi-sandbox/agents/ralph/orchestrator.yaml`, Lead Hare tier, interactive) that I can invoke from inside the kanban worktree (`npm run agent -- ralph/orchestrator --feature <feature-slug>`) to run a grill session, produce `.scratch/<feature-slug>/PRD.md`, and then break the PRD into vertical-slice issue files at `.scratch/<feature-slug>/issues/<NN>-<slug>.md`, so that I get from "I have an idea" to "the mesh has work to dispatch" inside one coherent feature branch.
...
- **Mesh-worktree manager (launcher concern).** At `npm run mesh -- --feature <slug>`, the launcher creates `feature/<feature-slug>` off `main` (if absent) and `git worktree add`s it at `~/.mesh-scratch/<project>/<feature-slug>/kanban/`. The launcher refuses to start the Kanban unless invoked from inside a worktree (Story #2b). The Kanban runs from that worktree. On mesh shutdown, the worktree stays around (so the user can resume next time without a re-create); cleaning it up is a separate `npm run mesh:cleanup -- --feature <slug>` invocation when the user is done with the feature.

- **Orchestrator recipe (`pi-sandbox/agents/ralph/orchestrator.yaml`).** Lead Hare tier, interactive, invoked from the kanban worktree as `npm run agent -- ralph/orchestrator --feature <slug>` *before* the mesh is launched. Drives a grill session, writes `.scratch/<feature-slug>/PRD.md`, then breaks the PRD into vertical-slice issue files at `.scratch/<feature-slug>/issues/<NN>-<slug>.md`. Tools palette includes `read`, `grep`, `find`, `glob`, plus `deferred_write` (so the user reviews the PRD + issues at end-of-turn before commit). The orchestrator does **not** delegate to Foremen and is **not** part of the mesh runtime — it is a planning-time precursor that produces the inputs the Kanban then dispatches against.
...
- **One feature, one branch, one worktree.** Everything for a feature lives on `feature/<feature-slug>`: the PRD, the issue files, the issue-close commits, the AFK Foreman merges. The kanban worktree at `~/.mesh-scratch/<project>/<feature-slug>/kanban/` is where the user authors (via the orchestrator or by hand) and where the Kanban dispatches Foremen. Per-issue Foreman worktrees are siblings (`~/.mesh-scratch/<project>/<feature-slug>/foreman-<id>/`). The user's main project-root checkout never holds the feature branch and is never touched by the mesh.
...
- The mesh launcher takes `--project <path>` and `--feature <slug>`. There is no `--source-branch` flag in V1 — the launcher always branches off `main` (a future flag could relax this if needed).
