...
Add a markdown-issue-driven mesh that runs alongside the existing stack, on the same protocol layer. The mesh is launched per-feature: `npm run mesh -- --project <path> --feature <feature-slug>` creates (or resumes) the mesh's own feature branch `feature/<feature-slug>` off `main` (or `--source-branch`) and a dedicated worktree at `~/.mesh-scratch/<project>/<feature-slug>/kanban/` checked out on that branch. A long-lived **Kanban** peer (non-LLM, like `human-relay.mjs`) runs from that worktree, watching the **Project**'s issue tree under `.scratch/<feature-slug>/issues/` (file convention defined by this PRD; see the Schema Decisions section) via an `issue-watcher` extension. Issue authoring happens on `main` (the **source branch**), where the user files / edits issue files normally; before each Foreman spawn the Kanban rebases `feature/<feature-slug>` onto `main` to pick up the latest issue authoring and code state, aborting on conflict (issue stays open with a comment, user resolves manually). When an issue becomes ready (`Status: ready-for-agent` or `Status: ready-for-human`), the Kanban spawns a **Foreman** (LLM, per-issue, ephemeral) that runs the **Ralph Loop** autonomously: claims the issue file (writes a `Claimed-by:` line), checks out a per-issue git worktree on `feature/<feature-slug>-<NN>-<slug>` *off the mesh's feature branch*, writes tests, runs them, fixes failures, commits. Reintegration takes one of two mesh-internal paths: **AFK** (`ready-for-agent`) auto-merges the per-issue branch into the **mesh's feature branch** and closes the issue; **HITL** (`ready-for-human`) emits a submission carrying the branch name to `human-relay` and exits — disposing the worktree releases the branch ref into the project's `.git/`, and the human QAs (typically by creating their own temp worktree), merges into `feature/<feature-slug>` themselves, and signals close via `respond_to_request approve`. **The mesh never commits to `main`.** When the user is ready to integrate, they merge `feature/<feature-slug>` into `main` themselves (treat it like a long-running PR). The branch lives entirely in the Project's local repo; the mesh issues no `git push`, no remote API calls, no hosted-Git interactions. The Foreman can `delegate` to **Workers** (specialist recipes — code review, type-check, etc.) mid-loop for ad-hoc help. When no ready issue files remain, the Kanban idles on its bus socket; no Foremen run; no model cost is incurred. The existing deferred-* stack is preserved in named subdirectories — both stacks ride the same bus protocol and **Habitat** materialiser.

## User Stories

(All actors are the developer driving AgentFactory unless otherwise noted.)

1. As a developer, I want to launch the Ralph-Loop mesh against my project for a specific feature (`npm run mesh -- --project ~/Projects/myapp --feature v1-ralph-loop-mesh`, with optional `--source-branch <ref>` defaulting to `main`), so that one mesh invocation owns one feature's work and I can run multiple meshes in parallel for different features without them stepping on each other.
2. As a developer, I want the mesh launch to fail loudly if the project has no `.scratch/<feature-slug>/issues/` directory on the source branch, so that I do not launch a mesh against an unconfigured feature.
2a. As a developer, I want the mesh launcher to create (or resume) `feature/<feature-slug>` off the source branch and check it out in a dedicated worktree at `~/.mesh-scratch/<project>/<feature-slug>/kanban/`, so that the Kanban runs in a working tree fully isolated from my own checkout — I can keep editing, switching branches, and running tests in the project root without affecting the mesh.
2b. As a developer, I want the mesh to **never commit to my source branch (`main` by default)**, so that the mesh's work-in-progress accumulates on `feature/<feature-slug>` only and I integrate it on my own schedule by merging that branch back myself.
2c. As a developer, I want the **Kanban** to rebase `feature/<feature-slug>` onto the source branch before each Foreman spawn (picking up my latest issue authoring and code changes), aborting with a clear message on conflict so I can resolve it manually, so that Foremen never start work against a stale view of the codebase.
3. As a developer, I want to create an issue file under `.scratch/<feature-slug>/issues/` *on the source branch* with clear acceptance criteria, so that an autonomous **Foreman** can pick it up after the next rebase-on-spawn.
...
8. As a developer, I want a **Foreman** to create a git worktree on a per-issue feature branch named after the issue (e.g., `feature/<feature-slug>-<NN>-<slug>`), branched **off the mesh's feature branch (`feature/<feature-slug>`), not off `main`**, so that the Foreman's work is isolated and stacks cleanly on top of any prior Foremen's work for the same feature.
...
11a. As a developer, when a **Foreman** completes work on a `ready-for-agent` (AFK) issue with passing tests, I want it to auto-merge its per-issue branch into the **mesh's feature branch** (`feature/<feature-slug>`, *not* `main`) and exit cleanly, so that AFK work accumulates on the mesh's branch without ever touching my source branch.
11b. As a developer, when a **Foreman** completes work on a `ready-for-human` (HITL) issue, I want it to emit a submission to `human-relay` carrying the branch name, project path, and test output, then exit (releasing the worktree so the per-issue branch becomes freely checkable), so that I can QA the branch using whatever tooling I prefer (typically `git worktree add /tmp/qa <branch>` to keep my project root undisturbed) and then merge it into `feature/<feature-slug>` myself.
...
11d. As a developer, after I've manually merged a HITL per-issue branch into `feature/<feature-slug>`, I want to signal `human-relay`'s `respond_to_request approve` action to close the issue (move file to `issues/closed/`, set `Status: closed`, append a closing note — all committed on `feature/<feature-slug>`, not `main`); for AFK, the same close happens automatically on `feature/<feature-slug>` right after the auto-merge succeeds.
11e. As a developer, when I judge the mesh's feature branch ready, I want to merge `feature/<feature-slug>` into my source branch (`main`) myself with whatever strategy I prefer (`git merge`, squash, rebase-then-merge, or a hosted-Git PR if I want one), so that the integration step is fully under my control and the mesh stays out of my source-branch commit history.
...
- **Mesh-worktree manager (launcher concern).** At `npm run mesh -- --feature <slug>`, the launcher creates `feature/<feature-slug>` off the source branch (or fast-forwards the existing one if it points at an ancestor of source) and `git worktree add`s it at `~/.mesh-scratch/<project>/<feature-slug>/kanban/`. The Kanban runs from that worktree. On mesh shutdown, the worktree stays around (so the user can resume next time without a re-create); cleaning it up is a separate `npm run mesh:cleanup -- --feature <slug>` invocation when the user is done with the feature.

- **Source-branch sync (Kanban concern).** Before each Foreman spawn, the Kanban runs `git rebase <source-branch>` from the kanban worktree. On clean rebase, proceed with spawn. On conflict: abort the rebase, comment on the queued issue file's `## Comments` section ("rebase conflict — please resolve `feature/<feature-slug>` against `<source-branch>` manually"), leave the issue's `Status:` unchanged, skip the spawn for this iteration. The rebase pulls in any new issue authoring or code changes the user has committed on the source branch since last rebase.

- **Per-issue Worktree manager (extension).** Owns the per-issue git worktree lifecycle for a Foreman: `git worktree add` (off `feature/<feature-slug>`) on Foreman start, `git worktree remove` on completion or abort. Owns the branch naming convention and the AFK reintegration step (local `git merge` into `feature/<feature-slug>` — done from the kanban worktree, not from the per-issue worktree). For HITL the manager only disposes the worktree — the branch ref persists in the project's `.git/` and the human checks it out (typically in their own temp worktree). No `git push`, no hosted-Git API calls. Pure-function core (decide branch name, decide path, decide reintegration mode) testable without a real repo.

- **Reintegration mode selection (Foreman logic).** At claim time, the Foreman reads the issue's `Status:` line: `ready-for-agent` selects auto-merge (local merge into the mesh's feature branch, no submission envelope, never touches `main`); `ready-for-human` selects emit-submission-then-exit (no merge, no push — disposing the worktree releases the branch ref for the human). Test failure in either mode aborts before reintegration.
...
- **Issue-close step.** Two triggers, one helper, both committed on `feature/<feature-slug>` (never on `main`). (1) AFK: the Foreman, on successful auto-merge, `git mv`s the issue file from `issues/<NN>-<slug>.md` to `issues/closed/<NN>-<slug>.md`, sets `Status: closed`, and appends a closing note under `## Comments`. (2) HITL: the human merges the per-issue branch into `feature/<feature-slug>` manually first (typically `git -C ~/.mesh-scratch/<project>/<feature-slug>/kanban merge feature/<feature-slug>-<NN>-<slug>`, or via a temp worktree), then signals close via `human-relay`'s `respond_to_request approve`; the supervisor inbound rail performs the same `git mv` + edits on `feature/<feature-slug>`. The "merge first, then approve" order is intentional — an open issue is more recoverable than a closed-but-unmerged one if the human forgets a step. The local per-issue branch ref is **not** deleted on approve (Story #15) — the mesh's branch-cleanup blast radius is bounded to the abort/reject case.
...
- The Foreman receives its mesh feature branch via a CLI flag (`--mesh-branch feature/<feature-slug>`, set by the Kanban) so the per-issue worktree branches off the right base and AFK reintegration knows where to merge. The source branch (default `main`) is the Kanban's concern, not the Foreman's — Foremen never read or write the source branch directly.
...
- **Mesh-worktree manager's lifecycle.** Given a source-branch ref and a feature slug: does it create or fast-forward `feature/<feature-slug>`, materialise the kanban worktree, and refuse to clobber an unrelated existing branch? Testable against a tmpdir git repo.
- **Source-branch rebase logic.** Given a kanban worktree and a source branch, does the rebase succeed-or-abort cleanly, and does an aborted rebase leave the worktree in a recoverable state? Testable with a tmpdir repo containing a constructed conflict.
