...
Add a markdown-issue-driven mesh that runs alongside the existing stack, on the same protocol layer. The unit of work is a **feature**: a coherent body of effort the user wants to ship as one merge into `main`. For each feature the user works in a dedicated git worktree (created off `main` at `feature/<feature-slug>`), and **everything for that feature lives on that branch**: the PRD, the issue files, the per-issue Foreman commits, the issue-close commits. The mesh is launched per-feature (`npm run mesh -- --project <path> --feature <feature-slug>`); the launcher creates `feature/<feature-slug>` off `main` (if absent) and `git worktree add`s it at `<project>/.mesh-features/<feature-slug>/kanban/`. **Running the mesh requires being in a worktree** — never operate against the project root checkout directly, so a colleague's parallel work on the same repo never collides with the mesh. **The mesh never commits to `main`.** When the user is ready to integrate, they merge `feature/<feature-slug>` → `main` themselves.

Authoring a feature is a separate step *before* the mesh runs: a new **orchestrator** pi recipe (interactive, Lead Hare tier) drives a grill session → PRD → vertical-slice issue files all written on the feature branch. The user `cd`s into the kanban worktree and runs the orchestrator (`npm run agent -- ralph/orchestrator --feature <feature-slug>`); the orchestrator interviews the user, writes `.scratch/<feature-slug>/PRD.md`, then breaks the PRD into `.scratch/<feature-slug>/issues/<NN>-<slug>.md` files. The user reviews and commits. Then `npm run mesh -- --feature <feature-slug>` starts the Kanban: a long-lived non-LLM peer (like `human-relay.mjs`) running from the kanban worktree, watching `.scratch/<feature-slug>/issues/` for `Status: ready-for-agent` or `ready-for-human` files, dispatching **Foremen** (LLM, per-issue, ephemeral) for each. A Foreman runs the **Ralph Loop**: claims the issue file (writes a `Claimed-by:` line), checks out a per-issue worktree on `feature/<feature-slug>-<NN>-<slug>` *off `feature/<feature-slug>`*, writes tests, runs them, fixes failures, commits. Reintegration takes one of two mesh-internal paths: **AFK** (`ready-for-agent`) auto-merges the per-issue branch into `feature/<feature-slug>` and closes the issue; **HITL** (`ready-for-human`) emits a submission carrying the branch name to `human-relay` and exits — disposing the worktree releases the per-issue branch ref, and the human QAs (in their own temp worktree, or in their main project root which can now check out the unowned per-issue branch), merges into `feature/<feature-slug>` themselves, and signals close via `respond_to_request approve`. The Foreman can `delegate` to **Workers** (specialist recipes — code review, type-check, etc.) mid-loop for ad-hoc help. The mesh issues no `git push`, no remote API calls, no hosted-Git interactions. When no ready issue files remain, the Kanban idles on its bus socket; no Foremen run; no model cost is incurred. The existing deferred-* stack is preserved in named subdirectories — both stacks ride the same bus protocol and **Habitat** materialiser.

## User Stories

(All actors are the developer driving AgentFactory unless otherwise noted.)

1. As a developer, I want to launch the Ralph-Loop mesh against my project for a specific feature (`npm run mesh -- --project ~/Projects/myapp --feature v1-ralph-loop-mesh`), so that one mesh invocation owns one feature's work and I can run multiple meshes in parallel for different features (and alongside colleagues working on the same repo) without anyone stepping on each other.
2. As a developer, in **runtime mode** (default — no `--init` flag), I want the mesh launcher to fail loudly if the project has no `.scratch/<feature-slug>/issues/` directory on `feature/<feature-slug>`, so that I do not launch the Kanban against an unauthored feature (run with `--init` first to populate it via the Orchestrator — see stories 2d/2e).
2a. As a developer, I want the mesh launcher to create (or resume) `feature/<feature-slug>` off `main` and `git worktree add` it at `<project>/.mesh-features/<feature-slug>/kanban/`, so that the Kanban runs in a worktree fully isolated from my project-root checkout and from any colleague's parallel work on the same repo.
2b. As a developer, I want **running the mesh to be permitted only from inside a worktree** (the launcher refuses to operate against the project root checkout directly), so that the per-feature isolation invariant cannot be silently bypassed.
2c. As a developer, I want the mesh to **never commit to `main`**, so that all feature work-in-progress accumulates on `feature/<feature-slug>` only and I integrate it into `main` on my own schedule (with whatever strategy I prefer — `git merge`, squash, rebase-then-merge, or a hosted-Git PR if I want one).
2d. As a developer, I want a new **orchestrator** pi recipe (`pi-sandbox/agents/ralph/orchestrator.yaml`, Lead Hare tier, interactive) that runs a grill session, produces `.scratch/<feature-slug>/PRD.md`, and then breaks the PRD into vertical-slice issue files at `.scratch/<feature-slug>/issues/<NN>-<slug>.md`, so that I get from "I have an idea" to "the mesh has work to dispatch" inside one coherent feature branch.
2e. As a developer, I want the **orchestrator** to write the PRD and issue files using deferred-write semantics (so I review the diff at end-of-turn before anything is committed), so that I can refine the orchestrator's framing of the feature before locking it in.
2f. As a developer, I want `npm run mesh -- --project <path> --feature <feature-slug> --init` (**init mode**) to create the kanban worktree AND immediately spawn the Orchestrator interactively in it, so that going from "I have an idea" to "PRD + issues drafted on the feature branch" is a single command. After the Orchestrator exits and I commit, I run the same command without `--init` to start the Kanban runtime.
2g. As a developer, I want the launcher to ensure `.mesh-features/` is in the project's `.gitignore` (adding it on `main` with a single commit if absent, or failing with a clear message if I'd rather edit it myself), so that the per-feature worktrees never get accidentally tracked or committed.
3. As a developer, I want to author or edit issue files at `.scratch/<feature-slug>/issues/<NN>-<slug>.md` **on `feature/<feature-slug>`** (typically by `cd`-ing into the kanban worktree, since that's where the branch is checked out), so that the issue-source and the mesh's work-target are the same branch and there is no cross-branch sync concern.
...
8. As a developer, I want a **Foreman** to create a git worktree on a per-issue feature branch named after the issue (e.g., `feature/<feature-slug>-<NN>-<slug>`), branched off `feature/<feature-slug>`, so that the Foreman's work is isolated and stacks cleanly on top of any prior Foremen's work for the same feature.
...
11a. As a developer, when a **Foreman** completes work on a `ready-for-agent` (AFK) issue with passing tests, I want it to auto-merge its per-issue branch into `feature/<feature-slug>` and exit cleanly, so that AFK work accumulates on the feature branch without ever touching `main`.
11b. As a developer, when a **Foreman** completes work on a `ready-for-human` (HITL) issue, I want it to emit a submission to `human-relay` carrying the branch name, project path, and test output, then exit (releasing the per-issue worktree so the branch becomes freely checkable), so that I can QA the branch using whichever tooling I prefer — either by creating my own temp worktree (`git worktree add /tmp/qa <branch>`), or by checking it out directly in my main project root (since the kanban worktree owns `feature/<feature-slug>`, not the per-issue branch, there is no conflict) — and then merge it into `feature/<feature-slug>` myself.
11c. As a developer, when a **Foreman**'s tests fail on any issue (AFK or HITL), I want it to abort without merging, so that broken work never pollutes the mesh's feature branch (and never reaches `main`).
11d. As a developer, after I've manually merged a HITL per-issue branch into `feature/<feature-slug>`, I want to signal `human-relay`'s `respond_to_request approve` action to close the issue (move file to `issues/closed/`, set `Status: closed`, append a closing note — all committed on `feature/<feature-slug>`); for AFK, the same close happens automatically on `feature/<feature-slug>` right after the auto-merge succeeds.
11e. As a developer, when I judge `feature/<feature-slug>` ready, I want to merge it into `main` myself with whatever strategy I prefer (`git merge`, squash, rebase-then-merge, or a hosted-Git PR if I want one), so that the integration step is fully under my control and the mesh stays out of `main`'s commit history.
...
- **Mesh-worktree manager (launcher concern).** Two modes:
  - **Init mode** (`npm run mesh -- --feature <slug> --init`): the launcher (1) ensures `.mesh-features/` is in the project's `.gitignore` on `main` (adds + commits if absent, or fails with a clear message if the user has unstaged changes that would block the auto-commit); (2) creates `feature/<feature-slug>` off `main`; (3) `git worktree add`s it at `<project>/.mesh-features/<feature-slug>/kanban/`; (4) spawns the Orchestrator (`pi-sandbox/agents/ralph/orchestrator.yaml`) interactively in the new worktree; (5) exits when the Orchestrator's pi session ends. The user reviews the deferred-write diff, commits the PRD + issues themselves, then re-invokes without `--init`.
  - **Runtime mode** (default, no `--init`): the launcher fails fast if the kanban worktree doesn't exist or if `.scratch/<feature-slug>/issues/` is missing/empty on `feature/<feature-slug>`. Otherwise it spawns the Kanban from the existing worktree.
  Either mode refuses to start unless invoked from inside a worktree of the project (Story #2b). On mesh shutdown the kanban worktree stays around so the user can resume next time without re-creating; cleaning it up is a separate `npm run mesh:cleanup -- --feature <slug>` invocation when the user is done with the feature.

- **Orchestrator recipe (`pi-sandbox/agents/ralph/orchestrator.yaml`).** Lead Hare tier, interactive, normally invoked by the launcher in `--init` mode (but also runnable manually as `npm run agent -- ralph/orchestrator --feature <slug>` from inside the kanban worktree if the user wants to re-author). Drives a grill session, writes `.scratch/<feature-slug>/PRD.md`, then breaks the PRD into vertical-slice issue files at `.scratch/<feature-slug>/issues/<NN>-<slug>.md`. Tools palette includes `read`, `grep`, `find`, `glob`, plus `deferred_write` (so the user reviews the PRD + issues at end-of-turn before commit). The orchestrator does **not** delegate to Foremen and is **not** part of the mesh runtime — it is a planning-time precursor that produces the inputs the Kanban then dispatches against.

- **Per-issue Worktree manager (extension).** Owns the per-issue git worktree lifecycle for a Foreman: `git worktree add` (off `feature/<feature-slug>`) on Foreman start, `git worktree remove` on completion or abort. Owns the branch naming convention and the AFK reintegration step (local `git merge` into `feature/<feature-slug>` — done from the kanban worktree, not from the per-issue worktree). For HITL the manager only disposes the worktree — the per-issue branch ref persists in the project's `.git/` and the human checks it out (in their own temp worktree, or in their main project root). No `git push`, no hosted-Git API calls. Pure-function core (decide branch name, decide path, decide reintegration mode) testable without a real repo.

- **Reintegration mode selection (Foreman logic).** At claim time, the Foreman reads the issue's `Status:` line: `ready-for-agent` selects auto-merge (local merge into `feature/<feature-slug>`, no submission envelope, never touches `main`); `ready-for-human` selects emit-submission-then-exit (no merge, no push — disposing the worktree releases the per-issue branch ref for the human). Test failure in either mode aborts before reintegration.
...
- **Issue-close step.** Two triggers, one helper, both committed on `feature/<feature-slug>` (never on `main`). (1) AFK: the Foreman, on successful auto-merge, `git mv`s the issue file from `issues/<NN>-<slug>.md` to `issues/closed/<NN>-<slug>.md`, sets `Status: closed`, and appends a closing note under `## Comments`. (2) HITL: the human merges the per-issue branch into `feature/<feature-slug>` manually first (in the kanban worktree, in a temp worktree, or in the project root), then signals close via `human-relay`'s `respond_to_request approve`; the supervisor inbound rail performs the same `git mv` + edits on `feature/<feature-slug>`. The "merge first, then approve" order is intentional — an open issue is more recoverable than a closed-but-unmerged one if the human forgets a step. The per-issue branch ref is **not** deleted on approve (Story #15) — the mesh's branch-cleanup blast radius is bounded to the abort/reject case.
...
- **Reintegration is path-specific, feature-branch-bounded, and remote-free.** AFK (`ready-for-agent`) auto-merges the per-issue branch into `feature/<feature-slug>` — the V1 deliverable for AFK is the merged commit on `feature/<feature-slug>`. HITL (`ready-for-human`) emits a submission to `human-relay` and disposes the worktree — the V1 deliverable for HITL is the per-issue branch ref in the project's `.git/` plus the bus submission. **Neither path ever commits to `main`**; the user integrates `feature/<feature-slug>` → `main` on their own schedule (treat it like a long-running PR). The mesh issues no `git push` and no hosted-Git API calls in V1. Cross-machine review and hosted-Git PRs layer on as opt-in V2 enhancements (Story #33).
- **One feature, one branch, one worktree.** Everything for a feature lives on `feature/<feature-slug>`: the PRD, the issue files, the issue-close commits, the AFK Foreman merges. The kanban worktree at `<project>/.mesh-features/<feature-slug>/kanban/` is where the user authors (via the orchestrator or by hand) and where the Kanban dispatches Foremen. Per-issue Foreman worktrees are siblings (`<project>/.mesh-features/<feature-slug>/foreman-<id>/`). The user's main project-root checkout never holds the feature branch and is never touched by the mesh.
...
- The Foreman receives its mesh feature branch via a CLI flag (`--mesh-branch feature/<feature-slug>`, set by the Kanban) so the per-issue worktree branches off the right base and AFK reintegration knows where to merge. Foremen never read or write `main` directly.
- The mesh launcher takes `--project <path>`, `--feature <slug>`, and an optional `--init` flag (init mode → run Orchestrator; default → run Kanban runtime). There is no `--source-branch` flag in V1 — the launcher always branches off `main` (a future flag could relax this if needed).
- The Foreman's `submitTo` is `human-relay` in V1, used only on the HITL path. AFK runs do not emit a submission envelope.
- The per-issue Worktree manager exposes `prepareWorktree(issuePath, projectPath, meshBranch) → {worktreePath, branchName, mode}` (where `mode` is `"auto-merge"` or `"branch-emit"`) and `disposeWorktree(worktreePath)` to the Foreman, called at session start and end respectively. The reintegration call is a separate method on the same module: `reintegrate(worktreePath, mode, meshBranch) → {mergedCommit?: sha} | {}`. AFK returns the `feature/<feature-slug>` commit sha after a local `git merge --ff-only` (or merge commit) into the mesh branch — performed from the kanban worktree, not the per-issue worktree. HITL returns an empty object — disposeWorktree is sufficient to release the branch ref for the human. No `git push` and no hosted-Git API calls.
...
- **Per-issue Worktree manager's lifecycle.** Given a project path, an issue path, and a mesh-branch ref: does it create a per-issue worktree branched off the mesh branch, AFK-merge back into the mesh branch on success, and clean up correctly on success and on abort? Testable against a tmpdir git repo (real git, no model).
- **Mesh-worktree manager's lifecycle.** Given a feature slug and a project path: does it create or fast-forward `feature/<feature-slug>` off `main`, materialise the kanban worktree at the expected path, refuse to clobber an unrelated existing branch, and refuse to start when not invoked from inside a worktree? Testable against a tmpdir git repo.
- **Orchestrator's PRD-and-issues output shape.** Given a fixture grill transcript, does the orchestrator's deferred-write queue contain a well-formed `PRD.md` and a coherent set of vertical-slice issue files at the right paths, with the right `Status:` and `Depends-on:` lines? Testable against the orchestrator's deferred-write queue without committing anything.
