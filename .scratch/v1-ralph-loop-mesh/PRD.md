...
15. As a developer, I want a rejected or aborted **Foreman** (test failure, HITL `revise`, HITL `reject`) to release its claim (remove the `Claimed-by:` line) and delete its remote feature branch (HITL only), so that the issue can be re-attempted and the remote does not accumulate stale branches.
