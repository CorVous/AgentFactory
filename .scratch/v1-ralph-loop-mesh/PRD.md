...
11b. As a developer, when a **Foreman** completes work on a `ready-for-human` (HITL) issue, I want it to push the feature branch to the remote and emit a submission to `human-relay` carrying the branch name, project path, and test output, then exit, so that I can fetch and QA the branch manually using whatever tooling I prefer.
11c. As a developer, when a **Foreman**'s tests fail on any issue (AFK or HITL), I want it to abort without merging or pushing, so that broken work never pollutes the workflow branch or the remote.
...
13. As a developer, for HITL issues, I want the submission to `human-relay` to be the entire QA surface (no PRs, no GitHub dependency), so that the mesh works against any project regardless of which hosted Git platform (if any) the project uses.
...
15. As a developer, I want a rejected or aborted **Foreman** (test failure, HITL `revise`, HITL `reject`) to release its claim (remove the `Claimed-by:` line) and delete its remote feature branch (HITL only — the abort case is the only one where the mesh created remote noise that should not stay), so that the issue can be re-attempted and the remote does not accumulate stale branches from work that did not land. The mesh does **not** delete the remote feature branch on `approve` — once the human has merged, the branch is a normal merged-feature-branch ref like any other in the repo, and cleanup is the human's responsibility (auto-delete-on-merge settings, periodic `git remote prune origin`, or manual `git push origin --delete`).
...
33. As a developer, I want a future PR-shepherd module (V2, optional) to layer on top of the V1 HITL path for projects that use a hosted Git platform — opening a PR after the Foreman pushes, watching it for review-requested events, comments, and CI failures, and feeding those back to a re-spawned Foreman — so that hosted-Git users can opt into PR-style review without that being baked into the V1 mesh.
...
- **Worktree manager (extension).** Owns the per-issue git worktree lifecycle for a Foreman: `git worktree add` (off the workflow branch) on Foreman start, `git worktree remove` on completion (auto-merge for AFK, push for HITL) or abort. Owns the branch naming convention and the reintegration step (local merge for AFK, `git push` for HITL — no PR-creation calls, no hosted-Git API calls). Pure-function core (decide branch name, decide path, decide reintegration mode) testable without a real repo.

- **Reintegration mode selection (Foreman logic).** At claim time, the Foreman reads the issue's `Status:` line: `ready-for-agent` selects auto-merge (no submission envelope, no remote push); `ready-for-human` selects push-then-emit-submission to `human-relay`. Test failure in either mode aborts before reintegration.
...
- **Submission payload (V1 branch variant, HITL only).** The `submission` bus envelope's payload schema gains a branch-mode variant carrying `branchRef`, `remoteName` (e.g., `origin`), `projectPath`, `issuePath` (relative to the project root, e.g. `.scratch/v1-ralph-loop-mesh/issues/01-kanban-script.md`), optional `testOutput`. Only the HITL path emits this envelope. No `prRef` field — the mesh doesn't open PRs. The supervisor inbound rail (`_lib/supervisor-inbox.ts`) handles it via the same action graph; in V1 only `human-relay` invokes those actions (per ADR-0004).

- **Issue-close step.** Two triggers, one helper. (1) AFK: the Foreman, on successful auto-merge, `git mv`s the issue file from `issues/<NN>-<slug>.md` to `issues/closed/<NN>-<slug>.md`, sets `Status: closed`, and appends a closing note under `## Comments`. (2) HITL: the human merges the feature branch manually first, then signals close via `human-relay`'s `respond_to_request approve`; the supervisor inbound rail performs the same `git mv` + edits on the workflow branch. The "merge first, then approve" order is intentional — an open issue is more recoverable than a closed-but-unmerged one if the human forgets a step. The remote feature branch is **not** deleted on approve (Story #15) — the mesh's branch-cleanup blast radius is bounded to the abort/reject case.
...
- **Reintegration is path-specific and GitHub-free.** AFK (`ready-for-agent`) auto-merges the feature branch into the workflow branch — the V1 deliverable for AFK is the merged commit. HITL (`ready-for-human`) pushes the branch to the remote and emits a submission carrying the branch name to `human-relay` — the V1 deliverable for HITL is the pushed branch + the bus submission. The mesh issues no calls to a hosted-Git API in V1. PR-shepherd richer ops (Story #33) layer on the HITL path in V2 for projects that opt into hosted-Git PRs.
...
- **Hosted-Git PR opening and watching.** V1's HITL path pushes a branch and emits its name to `human-relay`; the human QAs and merges manually using whatever tooling they prefer. The mesh issues no hosted-Git API calls (no `gh pr create`, no GitLab API, etc.). A future `pr-shepherd` peer that opts into hosted-Git PRs — opening one after push, watching for review-requested events, comments, and CI failures, feeding them back to a re-spawned Foreman — is V2 (see Story #33). V1 ships the PR-free baseline.
