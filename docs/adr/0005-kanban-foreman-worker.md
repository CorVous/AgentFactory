...
- The `submission` bus envelope's payload schema gains a branch-mode variant: `branchRef`, `remoteName` (e.g., `origin`), `projectPath`, `issuePath` (relative to the project root), optional `testOutput`. No `prRef` field — the mesh doesn't open PRs. Only the HITL path emits this envelope; AFK runs auto-merge directly and skip the submission entirely. The wire format stays bus-envelope-compatible.
