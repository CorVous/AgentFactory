...
## Amendment (2026-05-04) — Multiplexer relocates into a peer extension; launcher script becomes a thin shim

ADR-0007 collapses launches into recipes; ADR-0009 makes the host peer the sole owner of the PTY pool. Both together relocate every "launcher module" listed in this ADR's Decomposition table into a single peer-side extension `mesh-mux.ts` running inside the host peer's pi process. `scripts/launch-mesh.mjs` shrinks to an entry-point shim that resolves the host recipe and execs `scripts/run-agent.mjs <host> --is-host`. The launcher modules — `pty-pool`, `multiplexer`, `launcher-socket`, `decisions-queue`, `focus-controller`, `bus-tail`, `entry-resolver`, `topology-validator`, `topology` — survive as `.mjs` libraries in `scripts/_lib/` consumed by `mesh-mux.ts` via `createRequire` (the same pattern `launcher-bridge` already uses for `launcher-socket.mjs`). They are unchanged in shape; only their caller moves.

This addresses the K1/K2 trajectory deferred by ADR-0006: K1 (launcher footprint reduces; per-node spawn becomes a peer-extension responsibility) lands now; K2 (launcher itself loading "launcher-side extensions") is no longer relevant because the launcher *is* a peer extension. The "launcher itself becomes a process loading extensions" framing is replaced by "the host peer's process is the launcher; its extensions own the human surface."

The relocation does not relitigate this ADR's core decisions: every peer still runs a full pi TUI in its own PTY (the pool is just held by `mesh-mux` instead of `launch-mesh.mjs`); the focused peer's PTY still passes through to the launcher's stdout (now the host's stdout); intercept still reuses pi's `ctx.ui.confirm` on the focused peer. The "Top Supervisor recommended tier ≥ `LEAD_HARE_MODEL`" guidance becomes a hard requirement: the host peer is the top supervisor (per ADR-0007), and its LLM is the human's first review surface for `escalate`-from-top, so its tier choice matters more than ever. See ADR-0009 for the spawn protocol shape and ADR-0007 for the recipe/launch consolidation.
